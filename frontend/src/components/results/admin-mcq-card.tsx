...
import type { MCQQuestion, MCQAnswer } from "@/types/result"

interface AdminMCQCardProps {
    question: MCQQuestion
    answer?: MCQAnswer
    questionNumber: number
}

export const AdminMCQCard = ({ question, answer, questionNumber }: AdminMCQCardProps) => {
                        ...
                        {question.options.map((option: string, index: number) => (
                            ...
                            >
                                ...
                                {options[index] === answer?.selectedOption && options[index] !== question.correctOption && (
                                    <Badge className="ml-2 bg-red-100 text-red-800 text-xs">Student Answer</Badge>
                                )}
                                {options[index] === answer?.selectedOption && options[index] === question.correctOption && (
                                    <Badge className="ml-2 bg-green-100 text-green-800 text-xs">Student Answer (Correct)</Badge>
                                )}
                            </div>
                        ))}
                    </div>
